...
class gridMaze():
    def __init__(self, maze_bounds, maze_dims, std=10, sparsity=0, contiguous=True):
        ...
        focus_x = self.shape[0]//3 #np.random.randint(0,self.shape[1])
        focus_y = self.shape[1]//3  #np.random.randint(0,self.shape[1])
...
test = gridMaze([1200,800], [300,200], std=50, sparsity=0)
